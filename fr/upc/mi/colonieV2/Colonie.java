...
// matrice au cas ou
    private ArrayList<ArrayList<Integer>> matriceRelation(){
        ArrayList<ArrayList<Integer>> matrice=new ArrayList<>(crewmateList.size());
        for(List<Integer> a:matrice){
            a = new ArrayList<Integer>(crewmateList.size());
            for(int i=0;i<crewmateList.size();i++){
                a.set(i,0); // initialise a 0 la matrice
            }
        }

        for(int i=0;i<crewmateList.size();i++){
            for(Crewmate c: crewmateList.get(i).getRelations()){
                matrice.get(i).set(crewmateList.indexOf(c),1);
                matrice.get(crewmateList.indexOf(c)).set(i,1);
            }
        }
        return matrice;
    }
    ...
    private void affectationAuto() throws Exception{
       // ArrayList<Crewmate> sortedCrewmateList= new ArrayList<>();
        //ortedCrewmateList.addAll(crewmateList);
        Collections.sort(crewmateList);
